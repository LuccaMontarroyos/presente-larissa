...
export default function MomentsSection() {
  ...
  // No mobile: col-span-1 (metade da tela) ou col-span-2 (tela toda)
  const photos = [
    { src: "/foto-1-borogodo.jpeg", alt: "Eu namoro com um espetáculo", style: "col-span-1 row-span-2" },
    { src: "/foto-pipa.jpeg", alt: "Pipa", style: "col-span-2 md:col-span-2 row-span-2" }, // Larga
    ...
    { src: "/foto-wedo.jpeg", alt: "Wedo", style: "col-span-2 md:col-span-2 row-span-2" }, // Larga
  ...
  return (
    <section className="py-12 md:py-24 px-2 md:px-4 max-w-6xl mx-auto">
      ...
      {/* A Galeria Bento Grid - Ajustado para Mobile (grid-cols-2 e altura menor) */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 auto-rows-[120px] md:auto-rows-[200px]">
        {photos.map((photo, index) => (
          <motion.div
            ...
            className={`relative rounded-xl md:rounded-3xl overflow-hidden shadow-md md:shadow-lg border-2 md:border-4 border-white group ${photo.style}`}
          >
            <div className="relative w-full h-full bg-slate-200">
               ...
               <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-300 flex items-end p-2 md:p-4">
                 <p className="text-white text-xs md:text-base font-bold opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300 flex items-center gap-1 md:gap-2">
                   <Heart size={12} className="md:w-4 md:h-4" fill="currentColor" /> {photo.alt}
                 </p>
               </div>
            </div>
          </motion.div>
